...
export async function api(path:string,init:RequestInit={}){
  ...
  try{
    const r=await fetch(`${BASE}${path}`,{...init,headers:h,signal:c.signal});
...
export const riskApi={
  ...
  record:(id:string)=>api(`/blacklist/${id}`),
  ...
  dispute:(id:string,f:FormData)=>api(`/blacklist/${id}/dispute`,{method:'POST',body:f}),
  removalPreview:(id:string)=>api(`/blacklist/${id}/removal/preview`,{method:'POST'}),
  removalRequest:(id:string,p:any)=>api(`/blacklist/${id}/removal/request`,{method:'POST',body:JSON.stringify(p)}),
